...
int pesquisaRec(Registro *x, Apontador ap){
    if (ap == NULL) {
    ...
    if (x->chave < ap->r[i - 1].chave) 
        return pesquisaRec(x, ap->p[i - 1]);
    else
        return pesquisaRec(x, ap->p[i]);
}

int pesquisaArvoreB(ArvoreB *raiz, Registro *reg){
    if(raiz == NULL) return -1; 
    if(estaVazia(raiz)){
        printf("Arvore B Vazia!\n");
        return -1; 
    }
    return pesquisaRec(reg, *raiz); 
}
